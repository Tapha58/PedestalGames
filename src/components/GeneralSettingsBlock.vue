...
export default {
  ...
  methods: {
    ...
    //         }
    // },
    members_is_hidden: async function () {
      let response = await fetch('/app/wallgames/members_is_hidden/' + this.auth_data_url + '&group_id=' + this.common_settings.partner_group_id)
      if (response.ok) {
    ...
    start_game: async function () {
      ...
      if (this.game_type === 5 || this.game_type === 6) {
        if (!/{ссылка}/.test(this.settings.post_text)) {
          this.message_error = 'Отсутствует {ссылка} в тексте поста'
          setTimeout(this.clear_message, 5000)
          return
        }
      }
      ...
      if (this.required_join_partner_group_abc === 'b' || this.required_join_partner_group_abc === 'c') {
        ...
        if (await this.members_is_hidden()) {
          this.message_error = 'Скрыты подписчики в партнерской группе'
          setTimeout(this.clear_message, 5000)
          return
        }
      }
