  ...
  <div>
    ...
    <div v-if="!show_alert_successful_installation">
      ...
      >
          ...
          <v-col v-if="!from_pedestal && $vuetify.breakpoint.name === ('xs')" align="center" class="mt-3">
          ...
          <v-col v-if="from_pedestal" align="center">
            <h3 class="mb-1">Пьедестал. Игры</h3>
            <span>Это дополнение к приложению Пьедестал, позволяющее проводить интерактивные игры на стене сообщества. Небольшой нюанс: подключается этот функционал как отдельное приложение, но далее использовать его можно прямо из Пьедестала. Нажмите "подключить к сообществу" и выберите группу.</span><br>
            <v-btn @click="app_install" class="mt-3" color="primary" small>Подключить к сообществу</v-btn>
          </v-col>
      </v-sheet>
...
export default {
  ...
  mounted: async function () {
    ...
    this.auto_resize()
    // console.log(this.from_pedestal)
  },
  computed: {
    mobile: function () {
